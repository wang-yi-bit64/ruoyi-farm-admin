...
export function getColorPalettes(color: AnyColor, darkTheme = false, darkThemeMixColor = "#141414"):string[] {
	...
	if(darkTheme) {
		const darkPatterns = darkColorMap.map((item) => {
			const darkColor = colord(darkThemeMixColor).mix(colord(color), item.opacity).toHex();
...
/** 颜色混合
 * @param firstColor - 第一颜色
 * @param secondColor - 第二颜色
 * @param ratio - 第二个颜色占比
 */
 ...
 * @param color - 颜色
 */
export function getRgb0Color(color: string) {
